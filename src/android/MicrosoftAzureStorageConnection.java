...
public class MicrosoftAzureStorageConnection {
    ...
    public void uploadZipFile(String filepath, String ClientId, String ClientSecret, String TennantId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                File file = new File(filepath);
                if (file.exists()) {
                    String token = getAccessToken(ClientId, ClientSecret, TennantId);
                    String accessBlobToken = processToken(token);
                    uploadFileResumable(accessBlobToken, file, ClientId, ClientSecret, TennantId);
                } else {
    ...
    public static void uploadFileResumable(String bearerToken, File file, String ClientId, String ClientSecret, String TennantId) {
        ...
        try {
            ...
            if (!response.isSuccessful()) {
                ...
                if (response.code() == 308) {
                    handle308Status(response, file, ClientId, ClientSecret, TennantId);
                } else {
                    Log.e(TAG, "Error uploading the file, resuming...");
                    handleTimeoutWithRetries(request, file, ClientId, ClientSecret, TennantId);
                }
            }
        } catch (IOException e) {
    ...
    private static void handleTimeoutWithRetries(Request request, File file, String ClientId, String ClientSecret, String TennantId) {
        ...
        while (Instant.now().isBefore(end)) {
            try {
                request = handle401Status(request, ClientId, ClientSecret, TennantId);
                Response response = client.newCall(request).execute();
    ...
    private static Request handle401Status(Request request, String ClientId, String ClientSecret, String TennantId) {
        String token = getAccessToken(ClientId, ClientSecret, TennantId);
        String accessBlobToken = processToken(token);
    ...
    private static void handle308Status(Response response, File file, String ClientId, String ClientSecret, String TennantId) {
        ...
        try {
            ...
            if (rangeHeader != null) {
                ...
                Request newRequest = buildRequestWithHeaders(processToken(getAccessToken(ClientId, ClientSecret, TennantId)), file, newStartByte, file.length() - 1, chunkBody);
                ...
                if (newResponse.isSuccessful()) {
                    ...
                    handleTimeoutWithRetries(newRequest, file, ClientId, ClientSecret, TennantId);
                }
            }
        } catch (IOException e) {
